...
S1(config)#enable secret ^NqVqT$t7Jt8^A #définir un mot de passe pour le mode privilégié
...
R1(config-line)#password d8kS6tmVD%HJke
...
R1(config-ext-nacl)#permit ip 192.168.10.0 0.0.0.255 192.168.2.0 0.0.0.255
R1(config-ext-nacl)#permit ip host 192.168.40.80 0.0.0.255 192.168.1.0 0.0.0.255
R1(config-ext-nacl)#permit ip host 192.168.40.80 0.0.0.255 192.168.2.0 0.0.0.255
...
R1(config-ext-nacl)#deny ip 192.168.10.0 0.0.0.255 192.168.2.0 0.0.0.255
R1(config-ext-nacl)#deny ip host 192.168.40.80 192.168.1.0 0.0.0.255
R1(config-ext-nacl)#deny ip host 192.168.40.80 192.168.2.0 0.0.0.255
R1(config-ext-nacl)#permit ip 192.168.10.0 0.0.0.255 any
R1(config-ext-nacl)#permit ip 192.168.20.0 0.0.0.255 any
R1(config-ext-nacl)#exit
R1(config)#ip nat inside source list nat interface GigabitEthernet0/0/1 overload #déclarer la traduction d'adresse. Les paquets sortant du réseau auront comme adresse IP source l'adresse IP de l'interface GigabitEthernet0/0/1
...
R4(config-ext-nacl)#permit ip 192.168.2.0 0.0.0.255 192.168.10.0 0.0.0.255
R4(config-ext-nacl)#permit ip 192.168.2.0 0.0.0.255 host 192.168.40.80 
R4(config-ext-nacl)#exit
R4(config)#ip access-list extended nat
R4(config-ext-nacl)#deny ip 192.168.2.0 0.0.0.255 192.168.10.0 0.0.0.255
R4(config-ext-nacl)#deny ip 192.168.2.0 0.0.0.255 host 192.168.40.80 
R4(config-ext-nacl)#permit ip 192.168.2.0 0.0.0.255 any
R4(config-ext-nacl)#exit
...
R5(config-ext-nacl)#permit ip 192.168.1.0 0.0.0.255 host 192.168.40.80
...
R5(config-ext-nacl)#deny ip 192.168.1.0 0.0.0.255 host 192.168.40.80
...
R5(config-if)#exit
R5(config)#interface GigabitEthernet0/0/1
...
R3(config)#ip nat inside source list nat interface GigabitEthernet0/0 overload
```
